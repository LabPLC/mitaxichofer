package codigo.labplc.mx.mitaxichofer.trip.location;
...
public class GMapV2Direction {
	...
	private MitaxiTripActivity activity;
	private LatLng startPosition;
	
	public GMapV2Direction() {}

	public void getDocument(MitaxiTripActivity activity, LatLng start, LatLng end, String mode) {
		String url = "http://maps.googleapis.com/maps/api/directions/xml?" + "origin=" + start.latitude + "," + start.longitude
				+ "&destination=" + end.latitude + "," + end.longitude + "&sensor=false&units=metric&mode=" + mode;
		Log.d("url", url);
	...
	public class PostTask extends AsyncTask<Void, String, Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			...
			if(doc != null) {
				activity.addMarkerIntoMap(getDurationText(doc), startPosition);
				
				activity.drawRouteOnMap(doc);
			} else {
				Dialogues.Toast(activity, "Doc null", Toast.LENGTH_SHORT);
			}
		}
